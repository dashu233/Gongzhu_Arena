...
app=socketio.WSGIApp(sio, static_files={'/':'index.html','/favicon.ico':'logo.png','/asset':'asset','/src':'src'})
...
def login(sid,data):
    ...
    if data["room"] in ('T','Q'):
        ...
        try:
            ...
            rooms[roomid]=thisroom
            player_in_room[data['user']] = roomid
            sendmsg("loginsuc",{"name":data['user'],
                                    "room":roomid,"your_loc":0,"players":thisroom["players"]},sid=sid)
        except:
            log("",l=3)
            return 2
        finally:
...
def userchoice(sid,data):
    ...
    sendmsg('tableupdate',{'this_trick':thisroom["cards_played"][-1],'trick_start':thisroom['trick_start'][-1]},roomid=roomid)
    #下一张牌
